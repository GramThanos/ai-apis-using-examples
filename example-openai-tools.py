...
# Define a tool call
def tool_log_info(message: str) -> str:
	print('[LOG] ' + message)
	return True
...
available_tools = [
	{
		...
		'function': {
			'name': 'tool_log_info',
			'description': 'Log, in the form of a small title, what the user requested. It may return true if the logging was successful or false if it failed.',
			'parameters': {
				'type': 'object',
				'properties': {
					'title': {
						'type': 'string',
						'description': 'A small title associated with what the user requested',
					}
				},
				'required': ['title'],
			},
